#############################################################################
# Makefile for building: QT
# Generated by qmake (2.01a) (Qt 4.8.1) on: Sun May 7 22:03:38 2017
# Project:  QT.pro
# Template: app
# Command: /usr/bin/qmake-qt4 -spec /usr/share/qt4/mkspecs/linux-g++ -o Makefile QT.pro
#############################################################################

####### Compiler, tools and options

CC            = gcc
CXX           = g++
DEFINES       = -DQT_WEBKIT -DQT_NO_DEBUG -DQT_SQL_LIB -DQT_GUI_LIB -DQT_NETWORK_LIB -DQT_CORE_LIB -DQT_SHARED
CFLAGS        = -pipe -O2 -Wall -W -D_REENTRANT $(DEFINES)
CXXFLAGS      = -pipe -O2 -Wall -W -D_REENTRANT $(DEFINES)
INCPATH       = -I/usr/share/qt4/mkspecs/linux-g++ -I. -I/usr/include/qt4/QtCore -I/usr/include/qt4/QtNetwork -I/usr/include/qt4/QtGui -I/usr/include/qt4/QtSql -I/usr/include/qt4 -I. -I. -I.
LINK          = g++
LFLAGS        = -Wl,-O1
LIBS          = $(SUBLIBS)  -L/usr/lib/i386-linux-gnu -lQtSql -lQtGui -lQtNetwork -lQtCore -lpthread 
AR            = ar cqs
RANLIB        = 
QMAKE         = /usr/bin/qmake-qt4
TAR           = tar -cf
COMPRESS      = gzip -9f
COPY          = cp -f
SED           = sed
COPY_FILE     = $(COPY)
COPY_DIR      = $(COPY) -r
STRIP         = strip
INSTALL_FILE  = install -m 644 -p
INSTALL_DIR   = $(COPY_DIR)
INSTALL_PROGRAM = install -m 755 -p
DEL_FILE      = rm -f
SYMLINK       = ln -f -s
DEL_DIR       = rmdir
MOVE          = mv -f
CHK_DIR_EXISTS= test -d
MKDIR         = mkdir -p

####### Output directory

OBJECTS_DIR   = ./

####### Files

SOURCES       = application.c \
		CANopen.c \
		cmysplashscreen.cpp \
		CO_comm_helpers.c \
		CO_command.c \
		CO_driver.c \
		CO_Emergency.c \
		CO_HBconsumer.c \
		CO_Linux_tasks.c \
		CO_master.c \
		CO_NMT_Heartbeat.c \
		CO_OD.c \
		CO_OD_storage.c \
		CO_PDO.c \
		CO_SDO.c \
		CO_SDOmaster.c \
		CO_SYNC.c \
		CO_time.c \
		CO_trace.c \
		config.cpp \
		crc16-ccitt.c \
		dataset.cpp \
		globl_data.cpp \
		juxingkuang.cpp \
		learn.cpp \
		log.cpp \
		main.cpp \
		mainwindow.cpp \
		management.cpp \
		notice.cpp \
		product.cpp \
		qmyedit.cpp \
		sqlite.cpp \
		subwidget.cpp \
		systemset.cpp \
		thread_canopen.cpp \
		varify.cpp \
		widget.cpp \
		input.cpp \
		kboard.cpp \
		record.cpp moc_cmysplashscreen.cpp \
		moc_dataset.cpp \
		moc_learn.cpp \
		moc_log.cpp \
		moc_mainwindow.cpp \
		moc_management.cpp \
		moc_notice.cpp \
		moc_product.cpp \
		moc_qmyedit.cpp \
		moc_subwidget.cpp \
		moc_systemset.cpp \
		moc_thread_canopen.cpp \
		moc_varify.cpp \
		moc_widget.cpp \
		moc_input.cpp \
		moc_kboard.cpp \
		moc_record.cpp \
		qrc_Bofeng.cpp
OBJECTS       = application.o \
		CANopen.o \
		cmysplashscreen.o \
		CO_comm_helpers.o \
		CO_command.o \
		CO_driver.o \
		CO_Emergency.o \
		CO_HBconsumer.o \
		CO_Linux_tasks.o \
		CO_master.o \
		CO_NMT_Heartbeat.o \
		CO_OD.o \
		CO_OD_storage.o \
		CO_PDO.o \
		CO_SDO.o \
		CO_SDOmaster.o \
		CO_SYNC.o \
		CO_time.o \
		CO_trace.o \
		config.o \
		crc16-ccitt.o \
		dataset.o \
		globl_data.o \
		juxingkuang.o \
		learn.o \
		log.o \
		main.o \
		mainwindow.o \
		management.o \
		notice.o \
		product.o \
		qmyedit.o \
		sqlite.o \
		subwidget.o \
		systemset.o \
		thread_canopen.o \
		varify.o \
		widget.o \
		input.o \
		kboard.o \
		record.o \
		moc_cmysplashscreen.o \
		moc_dataset.o \
		moc_learn.o \
		moc_log.o \
		moc_mainwindow.o \
		moc_management.o \
		moc_notice.o \
		moc_product.o \
		moc_qmyedit.o \
		moc_subwidget.o \
		moc_systemset.o \
		moc_thread_canopen.o \
		moc_varify.o \
		moc_widget.o \
		moc_input.o \
		moc_kboard.o \
		moc_record.o \
		qrc_Bofeng.o
DIST          = /usr/share/qt4/mkspecs/common/unix.conf \
		/usr/share/qt4/mkspecs/common/linux.conf \
		/usr/share/qt4/mkspecs/common/gcc-base.conf \
		/usr/share/qt4/mkspecs/common/gcc-base-unix.conf \
		/usr/share/qt4/mkspecs/common/g++-base.conf \
		/usr/share/qt4/mkspecs/common/g++-unix.conf \
		/usr/share/qt4/mkspecs/qconfig.pri \
		/usr/share/qt4/mkspecs/modules/qt_phonon.pri \
		/usr/share/qt4/mkspecs/modules/qt_webkit_version.pri \
		/usr/share/qt4/mkspecs/features/qt_functions.prf \
		/usr/share/qt4/mkspecs/features/qt_config.prf \
		/usr/share/qt4/mkspecs/features/exclusive_builds.prf \
		/usr/share/qt4/mkspecs/features/default_pre.prf \
		/usr/share/qt4/mkspecs/features/release.prf \
		/usr/share/qt4/mkspecs/features/default_post.prf \
		/usr/share/qt4/mkspecs/features/unix/gdb_dwarf_index.prf \
		/usr/share/qt4/mkspecs/features/warn_on.prf \
		/usr/share/qt4/mkspecs/features/qt.prf \
		/usr/share/qt4/mkspecs/features/unix/thread.prf \
		/usr/share/qt4/mkspecs/features/moc.prf \
		/usr/share/qt4/mkspecs/features/resources.prf \
		/usr/share/qt4/mkspecs/features/uic.prf \
		/usr/share/qt4/mkspecs/features/yacc.prf \
		/usr/share/qt4/mkspecs/features/lex.prf \
		/usr/share/qt4/mkspecs/features/include_source_dir.prf \
		QT.pro
QMAKE_TARGET  = QT
DESTDIR       = 
TARGET        = QT

first: all
####### Implicit rules

.SUFFIXES: .o .c .cpp .cc .cxx .C

.cpp.o:
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o "$@" "$<"

.cc.o:
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o "$@" "$<"

.cxx.o:
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o "$@" "$<"

.C.o:
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o "$@" "$<"

.c.o:
	$(CC) -c $(CFLAGS) $(INCPATH) -o "$@" "$<"

####### Build rules

all: Makefile $(TARGET)

$(TARGET): ui_dataset.h ui_learn.h ui_log.h ui_mainwindow.h ui_management.h ui_notice.h ui_systemset.h ui_varify.h ui_widget.h ui_input.h ui_kboard.h $(OBJECTS)  
	$(LINK) $(LFLAGS) -o $(TARGET) $(OBJECTS) $(OBJCOMP) $(LIBS)

Makefile: QT.pro  /usr/share/qt4/mkspecs/linux-g++/qmake.conf /usr/share/qt4/mkspecs/common/unix.conf \
		/usr/share/qt4/mkspecs/common/linux.conf \
		/usr/share/qt4/mkspecs/common/gcc-base.conf \
		/usr/share/qt4/mkspecs/common/gcc-base-unix.conf \
		/usr/share/qt4/mkspecs/common/g++-base.conf \
		/usr/share/qt4/mkspecs/common/g++-unix.conf \
		/usr/share/qt4/mkspecs/qconfig.pri \
		/usr/share/qt4/mkspecs/modules/qt_phonon.pri \
		/usr/share/qt4/mkspecs/modules/qt_webkit_version.pri \
		/usr/share/qt4/mkspecs/features/qt_functions.prf \
		/usr/share/qt4/mkspecs/features/qt_config.prf \
		/usr/share/qt4/mkspecs/features/exclusive_builds.prf \
		/usr/share/qt4/mkspecs/features/default_pre.prf \
		/usr/share/qt4/mkspecs/features/release.prf \
		/usr/share/qt4/mkspecs/features/default_post.prf \
		/usr/share/qt4/mkspecs/features/unix/gdb_dwarf_index.prf \
		/usr/share/qt4/mkspecs/features/warn_on.prf \
		/usr/share/qt4/mkspecs/features/qt.prf \
		/usr/share/qt4/mkspecs/features/unix/thread.prf \
		/usr/share/qt4/mkspecs/features/moc.prf \
		/usr/share/qt4/mkspecs/features/resources.prf \
		/usr/share/qt4/mkspecs/features/uic.prf \
		/usr/share/qt4/mkspecs/features/yacc.prf \
		/usr/share/qt4/mkspecs/features/lex.prf \
		/usr/share/qt4/mkspecs/features/include_source_dir.prf \
		/usr/lib/i386-linux-gnu/libQtSql.prl \
		/usr/lib/i386-linux-gnu/libQtGui.prl \
		/usr/lib/i386-linux-gnu/libQtNetwork.prl \
		/usr/lib/i386-linux-gnu/libQtCore.prl
	$(QMAKE) -spec /usr/share/qt4/mkspecs/linux-g++ -o Makefile QT.pro
/usr/share/qt4/mkspecs/common/unix.conf:
/usr/share/qt4/mkspecs/common/linux.conf:
/usr/share/qt4/mkspecs/common/gcc-base.conf:
/usr/share/qt4/mkspecs/common/gcc-base-unix.conf:
/usr/share/qt4/mkspecs/common/g++-base.conf:
/usr/share/qt4/mkspecs/common/g++-unix.conf:
/usr/share/qt4/mkspecs/qconfig.pri:
/usr/share/qt4/mkspecs/modules/qt_phonon.pri:
/usr/share/qt4/mkspecs/modules/qt_webkit_version.pri:
/usr/share/qt4/mkspecs/features/qt_functions.prf:
/usr/share/qt4/mkspecs/features/qt_config.prf:
/usr/share/qt4/mkspecs/features/exclusive_builds.prf:
/usr/share/qt4/mkspecs/features/default_pre.prf:
/usr/share/qt4/mkspecs/features/release.prf:
/usr/share/qt4/mkspecs/features/default_post.prf:
/usr/share/qt4/mkspecs/features/unix/gdb_dwarf_index.prf:
/usr/share/qt4/mkspecs/features/warn_on.prf:
/usr/share/qt4/mkspecs/features/qt.prf:
/usr/share/qt4/mkspecs/features/unix/thread.prf:
/usr/share/qt4/mkspecs/features/moc.prf:
/usr/share/qt4/mkspecs/features/resources.prf:
/usr/share/qt4/mkspecs/features/uic.prf:
/usr/share/qt4/mkspecs/features/yacc.prf:
/usr/share/qt4/mkspecs/features/lex.prf:
/usr/share/qt4/mkspecs/features/include_source_dir.prf:
/usr/lib/i386-linux-gnu/libQtSql.prl:
/usr/lib/i386-linux-gnu/libQtGui.prl:
/usr/lib/i386-linux-gnu/libQtNetwork.prl:
/usr/lib/i386-linux-gnu/libQtCore.prl:
qmake:  FORCE
	@$(QMAKE) -spec /usr/share/qt4/mkspecs/linux-g++ -o Makefile QT.pro

dist: 
	@$(CHK_DIR_EXISTS) .tmp/QT1.0.0 || $(MKDIR) .tmp/QT1.0.0 
	$(COPY_FILE) --parents $(SOURCES) $(DIST) .tmp/QT1.0.0/ && $(COPY_FILE) --parents application.h CANopen.h cmysplashscreen.h CO_comm_helpers.h CO_command.h CO_driver.h CO_Emergency.h CO_HBconsumer.h CO_Linux_tasks.h CO_master.h CO_NMT_Heartbeat.h CO_OD.h CO_OD_storage.h CO_PDO.h CO_SDO.h CO_SDOmaster.h CO_SYNC.h CO_time.h CO_trace.h config.h crc16-ccitt.h dataset.h globl_data.h juxingkuang.h learn.h log.h mainwindow.h management.h Memorandum.h notice.h product.h qmyedit.h sqlite.h subwidget.h systemset.h thread_canopen.h varify.h widget.h input.h kboard.h record.h .tmp/QT1.0.0/ && $(COPY_FILE) --parents Bofeng.qrc .tmp/QT1.0.0/ && $(COPY_FILE) --parents application.c CANopen.c cmysplashscreen.cpp CO_comm_helpers.c CO_command.c CO_driver.c CO_Emergency.c CO_HBconsumer.c CO_Linux_tasks.c CO_master.c CO_NMT_Heartbeat.c CO_OD.c CO_OD_storage.c CO_PDO.c CO_SDO.c CO_SDOmaster.c CO_SYNC.c CO_time.c CO_trace.c config.cpp crc16-ccitt.c dataset.cpp globl_data.cpp juxingkuang.cpp learn.cpp log.cpp main.cpp mainwindow.cpp management.cpp notice.cpp product.cpp qmyedit.cpp sqlite.cpp subwidget.cpp systemset.cpp thread_canopen.cpp varify.cpp widget.cpp input.cpp kboard.cpp record.cpp .tmp/QT1.0.0/ && $(COPY_FILE) --parents dataset.ui learn.ui log.ui mainwindow.ui management.ui notice.ui systemset.ui varify.ui widget.ui input.ui kboard.ui .tmp/QT1.0.0/ && (cd `dirname .tmp/QT1.0.0` && $(TAR) QT1.0.0.tar QT1.0.0 && $(COMPRESS) QT1.0.0.tar) && $(MOVE) `dirname .tmp/QT1.0.0`/QT1.0.0.tar.gz . && $(DEL_FILE) -r .tmp/QT1.0.0


clean:compiler_clean 
	-$(DEL_FILE) $(OBJECTS)
	-$(DEL_FILE) *~ core *.core


####### Sub-libraries

distclean: clean
	-$(DEL_FILE) $(TARGET) 
	-$(DEL_FILE) Makefile


check: first

mocclean: compiler_moc_header_clean compiler_moc_source_clean

mocables: compiler_moc_header_make_all compiler_moc_source_make_all

compiler_moc_header_make_all: moc_cmysplashscreen.cpp moc_dataset.cpp moc_learn.cpp moc_log.cpp moc_mainwindow.cpp moc_management.cpp moc_notice.cpp moc_product.cpp moc_qmyedit.cpp moc_subwidget.cpp moc_systemset.cpp moc_thread_canopen.cpp moc_varify.cpp moc_widget.cpp moc_input.cpp moc_kboard.cpp moc_record.cpp
compiler_moc_header_clean:
	-$(DEL_FILE) moc_cmysplashscreen.cpp moc_dataset.cpp moc_learn.cpp moc_log.cpp moc_mainwindow.cpp moc_management.cpp moc_notice.cpp moc_product.cpp moc_qmyedit.cpp moc_subwidget.cpp moc_systemset.cpp moc_thread_canopen.cpp moc_varify.cpp moc_widget.cpp moc_input.cpp moc_kboard.cpp moc_record.cpp
moc_cmysplashscreen.cpp: cmysplashscreen.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) cmysplashscreen.h -o moc_cmysplashscreen.cpp

moc_dataset.cpp: dataset.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) dataset.h -o moc_dataset.cpp

moc_learn.cpp: globl_data.h \
		varify.h \
		qmyedit.h \
		config.h \
		kboard.h \
		ui_kboard.h \
		mainwindow.h \
		log.h \
		input.h \
		record.h \
		management.h \
		learn.h \
		product.h \
		notice.h \
		systemset.h \
		dataset.h \
		widget.h \
		sqlite.h \
		juxingkuang.h \
		learn.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) learn.h -o moc_learn.cpp

moc_log.cpp: globl_data.h \
		input.h \
		record.h \
		mainwindow.h \
		log.h \
		management.h \
		qmyedit.h \
		config.h \
		kboard.h \
		ui_kboard.h \
		learn.h \
		varify.h \
		product.h \
		notice.h \
		systemset.h \
		dataset.h \
		widget.h \
		sqlite.h \
		juxingkuang.h \
		log.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) log.h -o moc_log.cpp

moc_mainwindow.cpp: globl_data.h \
		log.h \
		input.h \
		record.h \
		mainwindow.h \
		management.h \
		qmyedit.h \
		config.h \
		kboard.h \
		ui_kboard.h \
		learn.h \
		varify.h \
		product.h \
		notice.h \
		systemset.h \
		dataset.h \
		widget.h \
		sqlite.h \
		juxingkuang.h \
		mainwindow.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) mainwindow.h -o moc_mainwindow.cpp

moc_management.cpp: qmyedit.h \
		config.h \
		kboard.h \
		ui_kboard.h \
		globl_data.h \
		learn.h \
		varify.h \
		mainwindow.h \
		log.h \
		input.h \
		record.h \
		management.h \
		product.h \
		notice.h \
		systemset.h \
		dataset.h \
		widget.h \
		sqlite.h \
		juxingkuang.h \
		management.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) management.h -o moc_management.cpp

moc_notice.cpp: notice.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) notice.h -o moc_notice.cpp

moc_product.cpp: product.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) product.h -o moc_product.cpp

moc_qmyedit.cpp: qmyedit.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) qmyedit.h -o moc_qmyedit.cpp

moc_subwidget.cpp: subwidget.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) subwidget.h -o moc_subwidget.cpp

moc_systemset.cpp: qmyedit.h \
		config.h \
		kboard.h \
		ui_kboard.h \
		globl_data.h \
		dataset.h \
		systemset.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) systemset.h -o moc_systemset.cpp

moc_thread_canopen.cpp: application.h \
		CANopen.h \
		CO_master.h \
		CO_driver.h \
		CO_SDO.h \
		CO_SDOmaster.h \
		CO_OD.h \
		CO_PDO.h \
		CO_Emergency.h \
		CO_SYNC.h \
		CO_NMT_Heartbeat.h \
		CO_HBconsumer.h \
		CO_trace.h \
		CO_OD_storage.h \
		CO_Linux_tasks.h \
		CO_time.h \
		CO_command.h \
		CO_comm_helpers.h \
		thread_canopen.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) thread_canopen.h -o moc_thread_canopen.cpp

moc_varify.cpp: qmyedit.h \
		config.h \
		kboard.h \
		ui_kboard.h \
		globl_data.h \
		mainwindow.h \
		log.h \
		input.h \
		record.h \
		management.h \
		learn.h \
		varify.h \
		product.h \
		notice.h \
		systemset.h \
		dataset.h \
		widget.h \
		sqlite.h \
		juxingkuang.h \
		varify.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) varify.h -o moc_varify.cpp

moc_widget.cpp: sqlite.h \
		juxingkuang.h \
		product.h \
		record.h \
		widget.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) widget.h -o moc_widget.cpp

moc_input.cpp: input.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) input.h -o moc_input.cpp

moc_kboard.cpp: ui_kboard.h \
		kboard.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) kboard.h -o moc_kboard.cpp

moc_record.cpp: record.h
	/usr/bin/moc-qt4 $(DEFINES) $(INCPATH) record.h -o moc_record.cpp

compiler_rcc_make_all: qrc_Bofeng.cpp
compiler_rcc_clean:
	-$(DEL_FILE) qrc_Bofeng.cpp
qrc_Bofeng.cpp: Bofeng.qrc \
		image/circle_red.png \
		image/circle_green.png \
		image/slider.png \
		image/Keyboard.png \
		image/Bofeng.png
	/usr/bin/rcc -name Bofeng Bofeng.qrc -o qrc_Bofeng.cpp

compiler_image_collection_make_all: qmake_image_collection.cpp
compiler_image_collection_clean:
	-$(DEL_FILE) qmake_image_collection.cpp
compiler_moc_source_make_all:
compiler_moc_source_clean:
compiler_uic_make_all: ui_dataset.h ui_learn.h ui_log.h ui_mainwindow.h ui_management.h ui_notice.h ui_systemset.h ui_varify.h ui_widget.h ui_input.h ui_kboard.h
compiler_uic_clean:
	-$(DEL_FILE) ui_dataset.h ui_learn.h ui_log.h ui_mainwindow.h ui_management.h ui_notice.h ui_systemset.h ui_varify.h ui_widget.h ui_input.h ui_kboard.h
ui_dataset.h: dataset.ui
	/usr/bin/uic-qt4 dataset.ui -o ui_dataset.h

ui_learn.h: learn.ui
	/usr/bin/uic-qt4 learn.ui -o ui_learn.h

ui_log.h: log.ui
	/usr/bin/uic-qt4 log.ui -o ui_log.h

ui_mainwindow.h: mainwindow.ui
	/usr/bin/uic-qt4 mainwindow.ui -o ui_mainwindow.h

ui_management.h: management.ui
	/usr/bin/uic-qt4 management.ui -o ui_management.h

ui_notice.h: notice.ui
	/usr/bin/uic-qt4 notice.ui -o ui_notice.h

ui_systemset.h: systemset.ui
	/usr/bin/uic-qt4 systemset.ui -o ui_systemset.h

ui_varify.h: varify.ui
	/usr/bin/uic-qt4 varify.ui -o ui_varify.h

ui_widget.h: widget.ui
	/usr/bin/uic-qt4 widget.ui -o ui_widget.h

ui_input.h: input.ui
	/usr/bin/uic-qt4 input.ui -o ui_input.h

ui_kboard.h: kboard.ui
	/usr/bin/uic-qt4 kboard.ui -o ui_kboard.h

compiler_yacc_decl_make_all:
compiler_yacc_decl_clean:
compiler_yacc_impl_make_all:
compiler_yacc_impl_clean:
compiler_lex_make_all:
compiler_lex_clean:
compiler_clean: compiler_moc_header_clean compiler_rcc_clean compiler_uic_clean 

####### Compile

application.o: application.c application.h \
		CANopen.h \
		CO_master.h \
		CO_driver.h \
		CO_SDO.h \
		CO_SDOmaster.h \
		CO_OD.h \
		CO_PDO.h \
		CO_Emergency.h \
		CO_SYNC.h \
		CO_NMT_Heartbeat.h \
		CO_HBconsumer.h \
		CO_trace.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o application.o application.c

CANopen.o: CANopen.c CANopen.h \
		CO_master.h \
		CO_driver.h \
		CO_SDO.h \
		CO_SDOmaster.h \
		CO_OD.h \
		CO_PDO.h \
		CO_Emergency.h \
		CO_SYNC.h \
		CO_NMT_Heartbeat.h \
		CO_HBconsumer.h \
		CO_trace.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CANopen.o CANopen.c

cmysplashscreen.o: cmysplashscreen.cpp cmysplashscreen.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o cmysplashscreen.o cmysplashscreen.cpp

CO_comm_helpers.o: CO_comm_helpers.c CO_comm_helpers.h \
		CO_driver.h \
		CO_SDO.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_comm_helpers.o CO_comm_helpers.c

CO_command.o: CO_command.c CANopen.h \
		CO_master.h \
		CO_driver.h \
		CO_SDO.h \
		CO_SDOmaster.h \
		CO_OD.h \
		CO_PDO.h \
		CO_Emergency.h \
		CO_SYNC.h \
		CO_NMT_Heartbeat.h \
		CO_HBconsumer.h \
		CO_trace.h \
		CO_command.h \
		CO_comm_helpers.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_command.o CO_command.c

CO_driver.o: CO_driver.c CO_driver.h \
		CO_Emergency.h \
		CO_SDO.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_driver.o CO_driver.c

CO_Emergency.o: CO_Emergency.c CO_driver.h \
		CO_Emergency.h \
		CO_SDO.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_Emergency.o CO_Emergency.c

CO_HBconsumer.o: CO_HBconsumer.c CO_driver.h \
		CO_SDO.h \
		CO_Emergency.h \
		CO_NMT_Heartbeat.h \
		CO_HBconsumer.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_HBconsumer.o CO_HBconsumer.c

CO_Linux_tasks.o: CO_Linux_tasks.c CANopen.h \
		CO_master.h \
		CO_driver.h \
		CO_SDO.h \
		CO_SDOmaster.h \
		CO_OD.h \
		CO_PDO.h \
		CO_Emergency.h \
		CO_SYNC.h \
		CO_NMT_Heartbeat.h \
		CO_HBconsumer.h \
		CO_trace.h \
		CO_Linux_tasks.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_Linux_tasks.o CO_Linux_tasks.c

CO_master.o: CO_master.c CO_master.h \
		CO_driver.h \
		CO_SDO.h \
		CO_SDOmaster.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_master.o CO_master.c

CO_NMT_Heartbeat.o: CO_NMT_Heartbeat.c CO_driver.h \
		CO_SDO.h \
		CO_Emergency.h \
		CO_NMT_Heartbeat.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_NMT_Heartbeat.o CO_NMT_Heartbeat.c

CO_OD.o: CO_OD.c CO_OD.h \
		CO_SDO.h \
		CO_driver.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_OD.o CO_OD.c

CO_OD_storage.o: CO_OD_storage.c CO_driver.h \
		CO_SDO.h \
		CO_Emergency.h \
		CO_OD_storage.h \
		crc16-ccitt.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_OD_storage.o CO_OD_storage.c

CO_PDO.o: CO_PDO.c CO_driver.h \
		CO_SDO.h \
		CO_Emergency.h \
		CO_NMT_Heartbeat.h \
		CO_SYNC.h \
		CO_PDO.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_PDO.o CO_PDO.c

CO_SDO.o: CO_SDO.c CO_SDO.h \
		CO_driver.h \
		crc16-ccitt.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_SDO.o CO_SDO.c

CO_SDOmaster.o: CO_SDOmaster.c CO_driver.h \
		CO_SDO.h \
		CO_SDOmaster.h \
		crc16-ccitt.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_SDOmaster.o CO_SDOmaster.c

CO_SYNC.o: CO_SYNC.c CO_driver.h \
		CO_SDO.h \
		CO_Emergency.h \
		CO_NMT_Heartbeat.h \
		CO_SYNC.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_SYNC.o CO_SYNC.c

CO_time.o: CO_time.c CO_time.h \
		CO_driver.h \
		CO_SDO.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_time.o CO_time.c

CO_trace.o: CO_trace.c CO_trace.h \
		CO_driver.h \
		CO_SDO.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o CO_trace.o CO_trace.c

config.o: config.cpp config.h \
		kboard.h \
		ui_kboard.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o config.o config.cpp

crc16-ccitt.o: crc16-ccitt.c crc16-ccitt.h
	$(CC) -c $(CFLAGS) $(INCPATH) -o crc16-ccitt.o crc16-ccitt.c

dataset.o: dataset.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o dataset.o dataset.cpp

globl_data.o: globl_data.cpp globl_data.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o globl_data.o globl_data.cpp

juxingkuang.o: juxingkuang.cpp juxingkuang.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o juxingkuang.o juxingkuang.cpp

learn.o: learn.cpp ui_learn.h \
		globl_data.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o learn.o learn.cpp

log.o: log.cpp log.h \
		globl_data.h \
		input.h \
		record.h \
		mainwindow.h \
		management.h \
		qmyedit.h \
		config.h \
		kboard.h \
		ui_kboard.h \
		learn.h \
		varify.h \
		product.h \
		notice.h \
		systemset.h \
		dataset.h \
		widget.h \
		sqlite.h \
		juxingkuang.h \
		ui_log.h \
		CANopen.h \
		CO_master.h \
		CO_driver.h \
		CO_SDO.h \
		CO_SDOmaster.h \
		CO_OD.h \
		CO_PDO.h \
		CO_Emergency.h \
		CO_SYNC.h \
		CO_NMT_Heartbeat.h \
		CO_HBconsumer.h \
		CO_trace.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o log.o log.cpp

main.o: main.cpp cmysplashscreen.h \
		globl_data.h \
		thread_canopen.h \
		application.h \
		CANopen.h \
		CO_master.h \
		CO_driver.h \
		CO_SDO.h \
		CO_SDOmaster.h \
		CO_OD.h \
		CO_PDO.h \
		CO_Emergency.h \
		CO_SYNC.h \
		CO_NMT_Heartbeat.h \
		CO_HBconsumer.h \
		CO_trace.h \
		CO_OD_storage.h \
		CO_Linux_tasks.h \
		CO_time.h \
		CO_command.h \
		CO_comm_helpers.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o main.o main.cpp

mainwindow.o: mainwindow.cpp ui_mainwindow.h \
		globl_data.h \
		CANopen.h \
		CO_master.h \
		CO_driver.h \
		CO_SDO.h \
		CO_SDOmaster.h \
		CO_OD.h \
		CO_PDO.h \
		CO_Emergency.h \
		CO_SYNC.h \
		CO_NMT_Heartbeat.h \
		CO_HBconsumer.h \
		CO_trace.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o mainwindow.o mainwindow.cpp

management.o: management.cpp ui_management.h \
		globl_data.h \
		CANopen.h \
		CO_master.h \
		CO_driver.h \
		CO_SDO.h \
		CO_SDOmaster.h \
		CO_OD.h \
		CO_PDO.h \
		CO_Emergency.h \
		CO_SYNC.h \
		CO_NMT_Heartbeat.h \
		CO_HBconsumer.h \
		CO_trace.h \
		input.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o management.o management.cpp

notice.o: notice.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o notice.o notice.cpp

product.o: product.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o product.o product.cpp

qmyedit.o: qmyedit.cpp qmyedit.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o qmyedit.o qmyedit.cpp

sqlite.o: sqlite.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o sqlite.o sqlite.cpp

subwidget.o: subwidget.cpp widget.h \
		sqlite.h \
		juxingkuang.h \
		product.h \
		record.h \
		ui_widget.h \
		subwidget.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o subwidget.o subwidget.cpp

systemset.o: systemset.cpp ui_systemset.h \
		globl_data.h \
		CANopen.h \
		CO_master.h \
		CO_driver.h \
		CO_SDO.h \
		CO_SDOmaster.h \
		CO_OD.h \
		CO_PDO.h \
		CO_Emergency.h \
		CO_SYNC.h \
		CO_NMT_Heartbeat.h \
		CO_HBconsumer.h \
		CO_trace.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o systemset.o systemset.cpp

thread_canopen.o: thread_canopen.cpp thread_canopen.h \
		application.h \
		CANopen.h \
		CO_master.h \
		CO_driver.h \
		CO_SDO.h \
		CO_SDOmaster.h \
		CO_OD.h \
		CO_PDO.h \
		CO_Emergency.h \
		CO_SYNC.h \
		CO_NMT_Heartbeat.h \
		CO_HBconsumer.h \
		CO_trace.h \
		CO_OD_storage.h \
		CO_Linux_tasks.h \
		CO_time.h \
		CO_command.h \
		CO_comm_helpers.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o thread_canopen.o thread_canopen.cpp

varify.o: varify.cpp ui_varify.h \
		globl_data.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o varify.o varify.cpp

widget.o: widget.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o widget.o widget.cpp

input.o: input.cpp input.h \
		ui_input.h \
		management.h \
		qmyedit.h \
		config.h \
		kboard.h \
		ui_kboard.h \
		globl_data.h \
		learn.h \
		varify.h \
		mainwindow.h \
		log.h \
		record.h \
		systemset.h \
		dataset.h \
		widget.h \
		sqlite.h \
		juxingkuang.h \
		product.h \
		notice.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o input.o input.cpp

kboard.o: kboard.cpp kboard.h \
		ui_kboard.h \
		config.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o kboard.o kboard.cpp

record.o: record.cpp record.h
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o record.o record.cpp

moc_cmysplashscreen.o: moc_cmysplashscreen.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_cmysplashscreen.o moc_cmysplashscreen.cpp

moc_dataset.o: moc_dataset.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_dataset.o moc_dataset.cpp

moc_learn.o: moc_learn.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_learn.o moc_learn.cpp

moc_log.o: moc_log.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_log.o moc_log.cpp

moc_mainwindow.o: moc_mainwindow.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_mainwindow.o moc_mainwindow.cpp

moc_management.o: moc_management.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_management.o moc_management.cpp

moc_notice.o: moc_notice.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_notice.o moc_notice.cpp

moc_product.o: moc_product.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_product.o moc_product.cpp

moc_qmyedit.o: moc_qmyedit.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_qmyedit.o moc_qmyedit.cpp

moc_subwidget.o: moc_subwidget.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_subwidget.o moc_subwidget.cpp

moc_systemset.o: moc_systemset.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_systemset.o moc_systemset.cpp

moc_thread_canopen.o: moc_thread_canopen.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_thread_canopen.o moc_thread_canopen.cpp

moc_varify.o: moc_varify.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_varify.o moc_varify.cpp

moc_widget.o: moc_widget.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_widget.o moc_widget.cpp

moc_input.o: moc_input.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_input.o moc_input.cpp

moc_kboard.o: moc_kboard.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_kboard.o moc_kboard.cpp

moc_record.o: moc_record.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o moc_record.o moc_record.cpp

qrc_Bofeng.o: qrc_Bofeng.cpp 
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -o qrc_Bofeng.o qrc_Bofeng.cpp

####### Install

install:   FORCE

uninstall:   FORCE

FORCE:


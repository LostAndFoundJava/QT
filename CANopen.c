...
/* Helper function for NMT master *********************************************/
    ...
    int8_t CO_sendNMTcommand(CO_t *CO, uint8_t command, uint8_t nodeID){
        ...
        /* Apply NMT command also to this node, if set so. */  //## 旧版本中在函数CO_NMT_receive(void *object, CO_CANrxMsg_t *msg)中
        if(nodeID == 0 || nodeID == CO->NMT->nodeId){          //## NMT命令
            switch(command){
...
/******************************************************************************/
CO_ReturnError_t CO_init(               //##  初始化最终得到（CO_t*）CO这个结构体

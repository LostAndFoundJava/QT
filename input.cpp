...
//#include<config.h>
input::input(QWidget *parent,bool num) :
    QWidget(parent),
    ui1(new Ui::input)
{
    ui1->setupUi(this);

    setEditUi(ui1->lineEdit);
    ...
    m_Display[0] = ui1->hz_1;
    m_Display[1] = ui1->hz_2;
    m_Display[2] = ui1->hz_3;
    m_Display[3] = ui1->hz_4;
    m_Display[4] = ui1->hz_5;
    m_Display[5] = ui1->hz_6;
    m_Display[6] = ui1->hz_7;
    m_Display[7] = ui1->hz_8;
    m_Display[8] = ui1->hz_9;
    m_Display[9] = ui1->hz_10;//汉字1-10的定义，将其统一放在一个数组中，方便后续代码。

    m_letter_a_z[0]=ui1->pushButton_a;
    m_letter_a_z[1]=ui1->pushButton_b;
    m_letter_a_z[2]=ui1->pushButton_c;
    m_letter_a_z[3]=ui1->pushButton_d;
    m_letter_a_z[4]=ui1->pushButton_e;
    m_letter_a_z[5]=ui1->pushButton_f;
    m_letter_a_z[6]=ui1->pushButton_g;
    m_letter_a_z[7]=ui1->pushButton_h;
    m_letter_a_z[8]=ui1->pushButton_i;
    m_letter_a_z[9]=ui1->pushButton_j;
    m_letter_a_z[10]=ui1->pushButton_k;
    m_letter_a_z[11]=ui1->pushButton_l;
    m_letter_a_z[12]=ui1->pushButton_m;
    m_letter_a_z[13]=ui1->pushButton_n;
    m_letter_a_z[14]=ui1->pushButton_o;
    m_letter_a_z[15]=ui1->pushButton_p;
    m_letter_a_z[16]=ui1->pushButton_q;
    m_letter_a_z[17]=ui1->pushButton_r;
    m_letter_a_z[18]=ui1->pushButton_s;
    m_letter_a_z[19]=ui1->pushButton_t;
    m_letter_a_z[20]=ui1->pushButton_u;
    m_letter_a_z[21]=ui1->pushButton_v;
    m_letter_a_z[22]=ui1->pushButton_w;
    m_letter_a_z[23]=ui1->pushButton_x;
    m_letter_a_z[24]=ui1->pushButton_y;
    m_letter_a_z[25]=ui1->pushButton_z;

    setWindowFlags(Qt::Tool|Qt::WindowStaysOnTopHint|Qt::FramelessWindowHint);
    /*窗口风格设置，这里是去除标题栏即蓝色部分*/
    if(num)
    {
        Flag_InputMode = NUMBER;   //输入格式变量
        Flag_Mid_Mode  = NUMBER;   //输入格式中转变量
        KeyBoardStartMode(NUMBER);
        ui1->pushButton_en_ch->setEnabled(false);
        ui1->pushButton_cap->setEnabled(false);
    }
    else
    {
        Flag_InputMode = LETTER;   //输入格式变量
        ...
        ui1->pushButton_en_ch->setEnabled(true);
        ui1->pushButton_cap->setEnabled(true);
    }
...
**********************************/
bool input::FindChinese(QString PinYin, QString &Chinese)
{
    QStringList lst;
    QString Line;
...
//此函数对所取得的字符所对应的汉字按字放在汉字显示框中。
{
    ...
    {
        m_Page_A=0;
        ui1->pushButton_next->setEnabled(false);
        ui1->pushButton_pre->setEnabled(false);
        return;
    }
    ...
    {
        ui1->pushButton_next->setEnabled(true);
    }
    ...
    {
        ui1->pushButton_pre->setEnabled(true);
    }
...
/******************鼠标拖动键盘处理部分*******************************/
void input::mouseMoveEvent(QMouseEvent *event)  //键盘拖动
{
...
void input::mousePressEvent(QMouseEvent *event) //读取键盘左击位置
{
...
void input::mouseReleaseEvent(QMouseEvent *event) //鼠标释放。
{
    Q_UNUSED(event);
...
/*********************通过键盘输入字符QLineEdit********************************/
void input::Input_letter_to_linEdit(const QString letter)
{
//    if(Flag_InputMode==LETTER)
    ...
    if (q != NULL)
    {
        e = dynamic_cast<QLineEdit *>(q);
        //dynamic_cast用于判断q的类型是否为QLineEdit如果是，那么e就为QLineEdit。
...
/*******存储输入的字符到拼音框中********************************/
void input::get_key_letter(QString letter)
{
    m_Page=0;
    if(k_letter_index++<6)
    {
...
void input::on_pushButton_en_ch_clicked()//中英切换button
{
...
void input::on_pushButton_cap_clicked()//大小写字母输入切换
{
    //初始化汉字部分的button
    ...
    if(flag==CAP_LETTER)
    {
        ...
        ui1->pushButton_cap->setText("cap");
        for(int i=0; i<26; i++)
        {
...
void input::clean_piy_and_hanzi_while_clicked(void)
...
void input::on_pushButton_q_clicked()
{
...
void input::on_pushButton_w_clicked()
{
    if(Flag_InputMode == HANZI)
    {
        get_key_letter("w");
...
void input::on_pushButton_e_clicked()
{
...
void input::on_pushButton_r_clicked()
{
    if(Flag_InputMode == HANZI)
    {
        get_key_letter("r");
...
void input::on_pushButton_t_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("t");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_y_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("y");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_u_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("u");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_i_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("i");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_o_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("o");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_p_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("p");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_a_clicked()
{
    if(Flag_InputMode == HANZI)
    {
        get_key_letter("a");
...
void input::on_pushButton_s_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("s");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_d_clicked()
{
    if(Flag_InputMode == HANZI)
     {
        get_key_letter("d");
...
void input::on_pushButton_f_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("f");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_g_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("g");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_h_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("h");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_j_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("j");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_k_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("k");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_l_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("l");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_z_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("z");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_x_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("x");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_c_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("c");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_v_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("v");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_b_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("b");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_n_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("n");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_m_clicked()
{
    if(Flag_InputMode == HANZI)
        get_key_letter("m");
    else if(Flag_InputMode == LETTER)
...
void input::on_pushButton_next_clicked()//下一页
{
    if(Flag_InputMode == HANZI)
    {
        if(m_Page<m_Page_A)
...
void input::on_pushButton_pre_clicked()//上一页
{
    if(m_Page==0) return;
    m_Page--;
    hanzi_zk_set();
...
void input::on_pushButton_del_clicked()//删除button
{
    if(m_CurPY=="")//如果当前拼音为空
    {
       // QWidget *q = QApplication::focusWidget();
        ...
        {
            e = dynamic_cast<QLineEdit *>(q);
            //dynamic_cast用于判断q的类型是否为QLineEdit如果是，
            //那么e就为QLineEdit。
...
void input::on_pushButton_sign_tan_clicked()
//感叹号！或者加号，如果是数字时为加号+
{
    Input_letter_to_linEdit(ui1->pushButton_sign_tan->text());
}
void input::on_pushButton_sign_wen_clicked()
//问号？或者减号，如果是数字时为-号
{
    Input_letter_to_linEdit(ui1->pushButton_sign_wen->text());
}
void input::on_pushButton_sign_dou_clicked()//逗号， 或者*，如果是数字时为*
{
    Input_letter_to_linEdit(ui1->pushButton_sign_dou->text());
}
void input::on_pushButton_sign_ju_clicked()//句号。或者/，如果是数字时为/
{
    Input_letter_to_linEdit(ui1->pushButton_sign_ju->text());
}
void input::on_pushButton_sign_fen_clicked()
//分号；或者等号，如果是数字时为=
{
    Input_letter_to_linEdit(ui1->pushButton_sign_fen->text());
}

void input::on_pushButton_space_clicked()//空格
{
    if(m_CurPY=="")//如果当前拼音为空
    {
...
void input::on_pushButton_Enter_clicked()
{
    if(m_CurPY=="")//如果当前拼音为空
    {
        Input_letter_to_linEdit("\r\n");
...
void input::KeyBoardStartMode(int Mode)   //进入键盘初始输入法
{
    if(Mode == CAP_LETTER)
    {
...
void input::KeyBoardHanziMode()           //键盘输入汉字模式
{
    ...
    //修改一些button
    ui1->pushButton_sign_tan->setText(tr("!"));
    ui1->pushButton_sign_wen->setText(tr("?"));
    ui1->pushButton_sign_dou->setText(tr(","));
    ui1->pushButton_sign_ju->setText(tr("."));
    ui1->pushButton_sign_fen->setText(tr(";"));
    for(int i=0;i<26;i++)
    {
        m_letter_a_z[i]->setEnabled(true);
    }
...
void input::KeyBoardNumMode()            //键盘输入数字模式
{
...
void input::KeyBoardLetterMode()              //键盘输入小写字母模式
{
    ...
    ui1-> pinyin ->setText(m_CurPY);//要用ui1指向前面所定义的指针
    hanzi_zk_set();
    ui1-> pushButton_en_ch->setText(tr("English"));
...
void input::setEditUi(QWidget *edit)
{
    this->edit = edit;
}

void input::Draw3dFrame(QRect &r, QPainter &dc)//画一个矩形框
{
    QRect rect = r;
